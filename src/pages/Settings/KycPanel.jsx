...
function LivenessCheck({ onCapture, captured, onRetake }) {
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      setPrompts(chosen); setPromptIdx(0); setPhase("active");
    } catch (err) {
  ...
  const captureFrame = useCallback(() => {
    ...
    if (promptIdx < prompts.length - 1) {
      setPromptIdx(i => i + 1); setPhase("active");
    } else {
      canvas.toBlob(blob => {
        ...
        stopStream(); setPhase("done"); onCapture(file);
      }, "image/jpeg", 0.92);
    }
  ...
  const beginCountdown = useCallback(() => {
    ...
    setPhase("countdown"); setCountdown(currentPrompt.duration);
    timerRef.current = setInterval(() => {
      setCountdown(prev => {
        if (prev <= 1) { clearInterval(timerRef.current); captureFrame(); return 0; }
        return prev - 1;
      });
    }, 1000);
  ...
  return (
    <div className="space-y-4">
      <div className="relative rounded-2xl overflow-hidden bg-stone-100 border border-stone-200 shadow-inner" style={{ aspectRatio: "4/3" }}>
          ...
          <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
            <svg width="100%" height="100%" viewBox="0 0 400 300" preserveAspectRatio="xMidYMid meet">
              ...
              <ellipse cx="200" cy="148" rx="98" ry="126" fill="none"
                ...
                strokeWidth="2.5" strokeDasharray={phase === "countdown" ? "10 4" : "0"} />
            </svg>
          </div>
        )}
        {phase === "countdown" && (
          <div className="absolute top-3 right-3 w-12 h-12 flex items-center justify-center">
            <svg className="absolute inset-0 -rotate-90" viewBox="0 0 48 48">
              ...
              <motion.circle cx="24" cy="24" r="20" fill="none" stroke="#f59e0b" strokeWidth="2.5" strokeLinecap="round"
        ...
        {(phase === "idle" || phase === "requesting") && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-stone-500 text-sm font-medium">{phase === "requesting" ? "Starting camera…" : "Camera ready"}</p>
          </div>
        )}
        {phase === "error" && (
